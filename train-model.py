...
def attention_pooling(inputs):
    ...
    attention_weights = tf.keras.layers.Softmax()(dense_layer)
...
y_train = np.array([1] * 1128+[0] * 1128)
y_test = np.array([1] * 282+[0] * 282)
p_train_selected=np.load('F:/a濛/New-data-ACPs/最新模型/最终模型/xtrain_pro_PE.npy')
p_test_selected=np.load('F:/a濛/New-data-ACPs/最新模型/最终模型/xtest_pro_PE.npy')


x_train_pro_PE = np.expand_dims(p_train_selected, axis=2)
x_test_pro_PE = np.expand_dims(p_test_selected, axis=2)

print(x_train_pro_PE.shape)
